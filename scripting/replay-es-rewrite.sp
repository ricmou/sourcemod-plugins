...
#define PLUGIN_VERSION "1.0.0" // Using semantic versioning: https://semver.org/
 ...
 * 
 * TODO: ADD cleaning of replay arrays;
 */
...
public Action Timer_OnCustomGhostPlaybackEnd(Handle timer, DataPack pack)
{
	...
	if ( IsPlayerAlive(ghost) )
	{
		TeleportEntity(g_iGhost[CUSTOM_GHOST], g_fSpawnPoint, view_as<float>({0.0, 0.0, 0.0}), view_as<float>({0.0, 0.0, 0.0}));
	}
...
public Action SM_UnloadReplays(int client, int args)
{
	...
	ReplyToCommand(client, "Unloaded replays.");
	return Plugin_Handled;
}
...
public Action SM_ReloadReplays(int client, int args)
{
	...
	ReplyToCommand(client, "Reloaded replays.");
	return Plugin_Handled;
}